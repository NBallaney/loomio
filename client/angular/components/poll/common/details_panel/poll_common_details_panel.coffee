...
CategoryInterface = require 'shared/interfaces/poll_categories_interface'
...
angular.module('loomioApp').directive 'pollCommonDetailsPanel', ->
  ...
  controller: ['$scope', 'clipboard', ($scope, clipboard) ->
    ...
    $scope.cat_namw2=""
    users = ""
    $scope.categoryFetched=false
    $scope.membersFetched = false
    $scope.executed = false
    $scope.completelyExecute = false
    $scope.completelyFinished = false
    $scope.definedname = false
    Records.groups.fetch().then((res) ->
      data = []
      for group in res.groups
        $scope.groupArray[group.id]=group.full_name
    )
    $scope.get_name_value = (user) ->
      Records.users.getUserName(user).then (data) ->
        $scope.usersArray[user]=data.name
    # $scope.get_name_value = (user) ->
    #   Records.users.getUserName.then (data) ->
    #     data.name
    ...
    if !$scope.membersFetched
      if $scope.poll.additionalData.apd_data2
        ...
        if $scope.poll.additionalData.apd_data1
          if $scope.poll.additionalData.apd_data1.user_ids
            userids = $scope.poll.additionalData.apd_data1.user_ids
        else
          if $scope.poll.additionalData.user_ids
            userids = $scope.poll.additionalData.user_ids
      
      if userids
        angular.forEach userids, (value) ->
           $scope.get_name_value(value)
    ...
    if !$scope.categoryFetched
      ...
      if $scope.poll.additionalData.apd_data2
        if $scope.poll.additionalData.apd_data1.poll_category_id
          Records.pollCategories.fetchById($scope.poll.additionalData.apd_data1.poll_category_id).then (respp) ->
            # console.log respp
            $scope.cat_name1 = respp.categories.name
            $scope.main_Cat1[respp.categories.id] = respp.categories.name
        if $scope.poll.additionalData.apd_data2.poll_category_id
          Records.pollCategories.fetchById($scope.poll.additionalData.apd_data2.poll_category_id).then (respp) ->
            # console.log respp
            $scope.cat_name2 = respp.categories.name
            $scope.main_Cat1[respp.categories.id] = respp.categories.name
            $scope.categoryFetched=true
        else
          $scope.categoryFetched=true
      else
        if $scope.poll.additionalData.apd_data1
          if $scope.poll.additionalData.apd_data1.poll_category_id
            Records.pollCategories.fetchById($scope.poll.additionalData.apd_data1.poll_category_id).then (respp) ->
              ...
              $scope.cat_name1 = respp.categories.name
              $scope.main_Cat1[respp.categories.id] = respp.categories.name
          if $scope.poll.additionalData.poll_category_id
              Records.pollCategories.fetchById($scope.poll.additionalData.poll_category_id).then (respp) ->
                # console.log respp
                $scope.cat_name2 = respp.categories.name
                $scope.main_Cat1[respp.categories.id] = respp.categories.name
                $scope.categoryFetched=true
      ...
      if $scope.poll.pollCategoryName == "Alliance Decision"
        ...
        if $scope.categoryFetched
          ...
          if $scope.poll.additionalData.apd_data2
            ...
            html_text+="Category Selected: #{$scope.main_Cat1[$scope.poll.additionalData.poll_category_id] ==  undefined && ($scope.poll.additionalData.poll_category_id=="all" && "All"|| "General") || $scope.main_Cat1[$scope.poll.additionalData.poll_category_id]}<br/>"
          ...
          if $scope.poll.additionalData.apd_data2
            ...
            pollcategoryname = $scope.main_Cat1[$scope.poll.additionalData.poll_category_id]
          ...
          if pollcategoryname == "Alliance Decision"
            ...
            if $scope.poll.additionalData 
              ...
              html_text1="Category to be modified : "+($scope.main_Cat1[polldetails.poll_category_id]!=undefined && $scope.main_Cat1[polldetails.poll_category_id] || "All")+"<br/>"+html_text1
